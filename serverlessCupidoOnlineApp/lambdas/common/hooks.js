...
const validateEventBody = async (state) => {
	...
	try {
		...
		console.error("yup validation error of event.body", error)
		state.exit = true
		state.response = {
			statusCode: 400,
...
const validatePaths = async (state) => {
	...
	try {
		...
		await pathSchema.validate(event.pathPatameters, { strict: true })
	} catch (error) {
		console.error("yup validation error of event parameters", error)
		state.exit = true
		state.response = {
			statusCode: 400,
			body: JSON.stringify({ error: error.message }),
